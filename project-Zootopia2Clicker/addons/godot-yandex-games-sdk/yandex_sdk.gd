...
func _game_initialized(args) -> void:
	...
	# --- SAFE ACCESS (Godot 4.6) ---
	app_id = str(env.app.id)

	lang = str(env.i18n.lang)
	tld = str(env.i18n.tld)

	# payload может быть undefined
	if env.payload != null:
		payload = str(env.payload)
	else:
		payload = ""

	print("LANG FROM YANDEX:", lang)

	is_game_initialized = true

	# важно — язык ставим только если он существует
	if lang != "":
